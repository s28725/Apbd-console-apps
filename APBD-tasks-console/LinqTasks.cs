using APBD_tasks_console.Models;
using System;
...
namespace APBD_tasks_console
{
     public static class LinqTasks
    {
        ...
        {
            var query1 = Emps.Select(emp => Tuple.Create(emp.Ename, emp.Job, emp.HireDate));
            var query2 = new[] { Tuple.Create("Brak wartości", (string)null, (DateTime?)null) };
        ...
        {
            var query = from e1 in Emps
                where Emps.Count(e2 => e1.Empno == e2.Mgr?.Empno) <= 1
                orderby e1.Ename ascending, e1.Salary descending
                select e1;
            return query.ToList();
        }
        ...
        {
            return Depts
                .GroupJoin(Emps, dept => dept.Deptno, emp => emp.Deptno, (dept, empGroup) => new { Dept = dept, Emps = empGroup })
                .Where(joinResult => joinResult.Emps.Count() == 5 || joinResult.Emps.Count() == 0)
                .OrderBy(joinResult => joinResult.Dept.Dname)
        ...
        public static IEnumerable<Dept> Task15()
        {
            return Emps.Where(emp => emp.Job.Contains("A"))
                ...
                .Where(deptGroup => deptGroup.Count() > 2)
                .OrderByDescending(deptGroup => deptGroup.Count())
                .Select(deptGroup => Depts.FirstOrDefault(dept => dept.Deptno == deptGroup.Key));
        }
